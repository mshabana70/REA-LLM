...
class APK_Recurse(APK):
    ...
    def set_node_summary(self, node_uid, summary):
        for node in self.funcs:
            if node["UID"] == node_uid:
                ...
                print("Node not found in functions list")
                return
    ...
    def traverse_recursive(self, node, depth=0, max_depth=4):
        ...
        if len(chd_summaries) > 1:
            ...
            clean_sums = [sum for sum in chd_summaries if sum != "No code found for function" or sum != "No summary generated for this function"] # Remove empty summaries
            if len(clean_sums) > num_sums_passed:
                joined_summaries = " ".join(clean_sums[:num_sums_passed])
            else:
                joined_summaries = " ".join(clean_sums)
    ...
    def summarize_apk(self):
        ...
        for ep_data in self.raw:
            ...
            print("\n\n++++++++++++++Summarizing entry point++++++++++++++")
            print("Current entry point name:", ep_name)
            ...
            if len(ep_children) > 0:
                ...
                print(f"Summary of current entry point function {func_name} without children: \n{ep_data['summary']}")
                continue
            ...
            summary = self.summarize_func(prompt_key, QUESTIONS, one_sum=True)
            ep_data["summary"] = summary
            print(f"Summary of current entry point function {func_name} and its children:\n{ep_data['summary']}")
